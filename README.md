...
| `--device cpu` | 强制用 CPU |
| `--interval 3` | 大约每隔几秒抽一帧。默认按时长自动：1 分钟内 2 秒，3 分钟内 3 秒，10 分钟内 6 秒，30 分钟内 12 秒，更长 20 秒 |
| `--frames 30` | 最多抽多少帧（默认按时长自动，上限 150；给了 `--interval` 时上限 300） |
| `--scene 0.2` | 场景切换灵敏度，越小越敏感 |
| `--no-dedupe` | 不去掉和前一帧几乎相同的画面（默认会去掉：缩略图上变化的像素不到 2% 就算重复） |
...
**想要更多画面？** `--interval 2` 让它每 2 秒抽一帧；`--scene 0.2` 让它对画面变化更敏感。
