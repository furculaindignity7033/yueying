...
def run(args, check=True) -> subprocess.CompletedProcess:
    ...
    if check and p.returncode != 0:
        raise RuntimeError("ffmpeg 失败: " + " ".join(cmd) + "\n" + p.stderr[-2000:])
    return p


def probe(path: str) -> dict:
    """用 ffmpeg -i 读时长/分辨率/有没有音轨和字幕轨（不依赖 ffprobe）。"""
    p = run(["-i", path], check=False)
    err = p.stderr
    ...
    if not info["duration"] and not info["width"]:
        raise RuntimeError("ffmpeg 读不了这个文件：" + path + "\n" + err[-800:])
    return info


def extract_audio(video: str, wav: str) -> None:
    """抽成 16k 单声道 wav，给语音识别用。"""
    run(["-i", video, "-vn", "-ac", "1", "-ar", "16000", "-c:a", "pcm_s16le", wav])


def extract_embedded_subtitle(video: str, out_srt: str) -> bool:
    """把视频内嵌的第一条字幕轨导成 srt，成功返回 True。"""
    p = run(["-i", video, "-map", "0:s:0", "-c:s", "srt", out_srt], check=False)
